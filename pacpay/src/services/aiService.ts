...
const generateFallbackResponse = (userMessage: string, context: AIFinancialContext): string => {
  ...
  return `⚠️ AI Agent Manager is currently offline. I cannot process specific market data or financial queries at this time. However, here is your most recent account summary:
  
Your Financial Profile:
- Savings Rate: ${(context.savingsRate * 100).toFixed(1)}%
- Budget Usage: ${((context.currentSpending / context.monthlyBudget) * 100).toFixed(1)}%

Recommendation: Since I'm offline, please review your manual budget or check again later for strategic advice. You are currently at Level ${context.level}.`;
};
